...
- **README: a structured four-row badge block at the top.** Row 1 is repo
  ...
  brand colors (Zsh, Bash, Lua, TOML; Neovim, Vim, tmux, Starship, Git). Tools
  with no simpleicon (`mise`, `lazygit`, `jujutsu`, `sesh`) get a neutral badge.
  Vim is the `vim/vimrc` fallback editor for boxes with no nvim, not just the
  `vim=nvim` alias; the tooling row now covers every tool Core ships a dedicated
  config for. Row 1 leads with a `dotgibson`
