...
- **`mise/config.toml`: `ruby` 3.4 → 4.0, `lua` 5.4 → 5.5.** Both verified resolvable before
  ...
  **How this surfaced is the part worth recording.** The change was found as an _uncommitted
  ...
  Core. That tree is regenerated by `scripts/sync-core.sh`, so the edit was on course to be
  silently reverted on the next sync — and `core-integrity` reports such a tree as TAMPERED.
