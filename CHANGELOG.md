...
- **`core-doctor` reported `✗ git-absorb` on the whole Debian family, for a tool that was
  ...
  **Fork budget.** `PATH` is probed first, so a box that never had the bug (Arch, Alpine,
  ...
  is cached in `_CORE_GIT_EXEC_PATH` for the life of one report — a test asserts
  `--exec-path` is invoked exactly once across a `-v` run, and another asserts git is never
  invoked at all when the subcommand is on `PATH`.
