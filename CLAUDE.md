...
- **The README hero tape is generated, not typed.** `assets/demo.tape` is rendered by
  ...
  `make gen-hero-tape-fleet` writes their tapes; seven of their gifs are committed in their
  repos (#948, filmed on rootless chroots — `assets/README.md`, "Filming a sibling hero without
  its box"), openSUSE waits on #950 reaching its vendored `core/`, and MacBook needs a Mac.
...
- `/os-package-availability` — audit an OS repo's package list for renamed/dropped/moved
  ...
  `@v7` caller (the `lint-call.yml` idiom) instead of a per-repo copy.
