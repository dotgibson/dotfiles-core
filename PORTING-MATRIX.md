...
5. **Commit steps 2–4 first** (`git add -A && git commit -m "os: the <Distro> layer"`):
   ...
   failure and may predate `--strict` — so its summary line is the verdict:
   `updated 1   skipped 0   failed 0` for the one target, and nothing else counts;
   that `repos:` footer exists since v4.1.0, so an older exact freeze cannot be judged
   this way — and the binding floor is v4.15.1 anyway, the first release whose sync
   MATERIALIZES rather than `git subtree pull --squash`, which cannot update a
   materialized `core/` at all)
...
**This table is rendered from `os/<os>.capabilities`** (#686). It began life the other way
round — as the transcription source those declarations were authored from (#667) — and
once seven repos declared (the two Role repos have no OS band and inherit the OS layer's
table) the declarations became the live files, held to the schema by `audit-core.sh` §9c,
...
openSUSE's `dup`-vs-`up` split and the staged editions — Fedora Atomic (Silverblue /
Kinoite / bootc, `rpm-ostree`) and openSUSE's transactional MicroOS / Aeon / Kalpa
(`transactional-update`) — rendered as labelled values in one column, and Debian's Kali
lane (its own column). The table gained its **macOS and Fedora columns** in #664: they had been
...
it cheaply at all, pays for a real resolve rather than the fast wrong answer. An atomic
host cannot answer it as a user at all (`rpm-ostree upgrade --check` is root-only), so its
declaration carries no count verb and the cell shows the **staged** probe the nudge asks
instead — `(staged?)`: exit status says whether a deployment is waiting for the reboot.

<!-- core:porting-matrix:gen commands -->

| Action        | macOS (brew)                  | Fedora (dnf)                                                                                           | Arch                                 | openSUSE                                                                                                                       | Alpine                       | Gentoo                          | Kali (apt)                      | Debian/Ubuntu (apt)             |
| ------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- | ------------------------------- | ------------------------------- | ------------------------------- |
| refresh       | `brew update`                 | Workstation: `sudo dnf check-update` · Atomic: `sudo rpm-ostree refresh-md`³⁵                          | `sudo pacman -Sy`²³                  | `sudo zypper refresh`                                                                                                          | `doas apk update`            | `sudo emerge --sync`            | `sudo apt-get update`           | `sudo apt-get update`           |
| upgrade       | `brew upgrade`                | Workstation: `sudo dnf upgrade --refresh` · Atomic: `sudo rpm-ostree upgrade`                          | `sudo pacman -Syu`                   | Leap: `sudo zypper up` · Tumbleweed: `sudo zypper dup` · Transactional: `sudo transactional-update dup`                        | `doas apk upgrade`           | `sudo emerge -auvDN @world`     | `sudo apt-get full-upgrade`     | `sudo apt-get full-upgrade`     |
...
| owns-file     | `brew which-formula <path>`³⁸ | Workstation: `dnf provides <path>` · Atomic: `rpm -qf <path>`                                          | `pacman -Qo <path>`                  | `zypper se --provides <path>`                                                                                                  | `apk info --who-owns <path>` | `equery belongs <path>`         | `dpkg -S <path>`                | `dpkg -S <path>`                |
...
**This table is rendered by `scripts/gen-porting-matrix.sh`** (#686), and each cell is one
of two kinds. A cell naming a package the repo **installs** is _derived_ from that repo's
...
no ³ fallback is needed there. **The cell carries ²¹ because `dotfiles-openSUSE` does not
install it** — the name is where `uv` comes from when you opt in, not something the bootstrap
puts on the box (#836). Arch, Alpine and Gentoo do install theirs, which is why only this
column is marked. **This is the same failure the paragraph above describes**:
