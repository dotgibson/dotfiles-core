...
#     blib_write_relink_stamp "$DOTFILES" full   # last, so an aborted run leaves the old stamp
#   }
#   provision() {
...
# blib_write_relink_stamp <dotfiles> <full|links-only> — record, ON THE HOST, which Core
# this box's symlinks were last wired against. core.lock says what a REPO vendored; nothing
# said what a BOX had relinked, so a contract change (#667 → #763) had to guess when the
# fleet had re-bootstrapped. core-doctor compares this stamp against the checkout's
# core.lock, and 60-update.zsh nudges on a mismatch.
#
# HOST STATE, NOT VENDORED: $XDG_STATE_HOME/dotfiles-core/, the directory the whatsnew and
# welcome stamps already live in. Same key=value shape as core.lock and os.capabilities —
# READ, NEVER SOURCED; the values are copied verbatim, so nothing in them is ever expanded.
#
# The caller decides WHETHER a run earned a stamp (blib_main: not on --dry-run, not on a
# --only/--skip partial wiring); this decides only what it says. Written to a temp file and
# renamed, so a run that dies mid-write leaves the previous stamp, never a torn one — and a
# run that aborts before it gets here leaves the previous stamp too, which is the point.
# No readable core.lock (Core itself, an unsynced checkout): nothing to compare against,
# so no stamp. Returns 0 either way — a stamp is a record, never a reason to fail a run.
blib_write_relink_stamp() {
  local dotfiles="$1" mode="$2" lock line k v sha="" tag="" ver="" libver="" dir stamp tmp body old
  lock="$dotfiles/core.lock"
  [[ -r "$lock" ]] || return 0
  # Physical path: the zsh reader derives the checkout from a :A-resolved symlink, so a
  # logical path through a symlinked parent would read as "another checkout".
  dotfiles="$(cd "$dotfiles" 2>/dev/null && pwd -P)" || dotfiles="$1"
  if _blib_dry; then
    blib_say "(dry run) would stamp this host's relink"
    return 0
  fi
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    k="${line%%=*}" v="${line#*=}"
    case "$k" in
    core_sha) [[ -n "$sha" ]] || sha="$v" ;;
    core_tag) [[ -n "$tag" ]] || tag="$v" ;;
    core_version) [[ -n "$ver" ]] || ver="$v" ;;
    esac
  done <"$lock"
  ...
  tmp="$stamp.tmp.$$"
  body="$(printf 'core_sha=%s\ncore_tag=%s\ncore_version=%s\nlib_version=%s\nmode=%s\ndotfiles=%s' \
    "$sha" "$tag" "$ver" "$libver" "$mode" "$dotfiles")"
  # A SECOND IDENTICAL RUN TOUCHES NOTHING. The scaffolded test/check-links.sh shims mkdir/mv
  # on a bootstrap's second run and reads any call as a change (#999), so a stamp rewritten
  # every run would red every OS repo's idempotency witness. Compared WITHOUT linked_at —
  # which therefore records when this box FIRST wired this Core in this mode, not the
  # latest re-run — through builtins only (read, [[ ]]), never a shimmed command.
  if [[ -r "$stamp" ]]; then
    old=""
    while IFS= read -r line || [[ -n "$line" ]]; do
      case "$line" in \#* | linked_at=*) continue ;; esac
      old="${old:+$old
}$line"
    done <"$stamp"
    if [[ "$old" == "$body" ]]; then
      blib_ok "relink stamp current: ${tag:-${sha:0:12}} (${mode})"
      return 0
    fi
  fi
  [[ -d "$dir" ]] || mkdir -p "$dir" 2>/dev/null || {
    blib_warn "could not create $dir — this host's relink is not stamped"
    return 0
  }
  if {
    printf '# GENERATED by bootstrap.sh (core/lib/bootstrap-lib.sh) — this HOST last relinked here.\n'
    printf '# Read, never sourced, by core-doctor and the shell-start nudge. Never hand-edit.\n'
    printf '%s\nlinked_at=%s\n' "$body" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  } >"$tmp" 2>/dev/null && mv -f "$tmp" "$stamp" 2>/dev/null; then
    blib_ok "relink stamped: ${tag:-${sha:0:12}} (${mode})"
  else
    rm -f "$tmp" 2>/dev/null
    blib_warn "could not write $stamp — this host's relink is not stamped"
  fi
  return 0
}
...
#
# THE RELINK STAMP (#1154): a run that wired everything ends by writing
# $XDG_STATE_HOME/dotfiles-core/bootstrap.lock (blib_write_relink_stamp) — which Core this
# host last relinked against. Not on --dry-run, not on --only/--skip, not on an abort.
#
...
blib_main() {
  ...
  blib_wire_summary
  # The host's relink stamp (#1154) — reached only by a run that wired everything: an abort
  ...
  if ((_bm_dry == 0)); then
    if [[ -n "$_bm_only$_bm_skip" ]]; then
      ...
      blib_write_relink_stamp "$DOTFILES" full
    fi
  fi
