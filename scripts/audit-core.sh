...
if ! ((SCOPE_SHELL)); then
  ...
  pf_fail=0
  # Built from fragments so this line cannot match its own pattern — the scanner reads
  # every tracked shell script, including this one.
  pf_re="(printf|echo)[^|]*[|][[:space:]]*(grep -[a-zA-Z]*q|head([[:space:]]|\$)|awk[^|]*exit)"
  while IFS= read -r pf_f; do
    [ -n "$pf_f" ] || continue
    # An actual `set` enabling it — not the mere word. A file that only MENTIONS pipefail
    # in a comment is not running under it, and scanning it would report safe code.
    grep -qE '^[[:space:]]*set[[:space:]]+-[a-zA-Z]*o[[:space:]]+pipefail' "$pf_f" 2>/dev/null || continue
    while IFS= read -r pf_hit; do
      [ -n "$pf_hit" ] || continue
      fail "pipefail: $pf_f:${pf_hit%%:*} — shell-string producer piped into an early-exiting reader; use a herestring"
      pf_fail=1
    done <<EOF
$(grep -nE "$pf_re" "$pf_f" 2>/dev/null | awk -F: '{ l = $0; sub(/^[0-9]+:/, "", l); if (l !~ /^[[:space:]]*#/) print }')
EOF
  done <<EOF
