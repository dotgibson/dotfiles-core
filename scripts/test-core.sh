...
#     it the answer is re-derived per git-* row and per renderer. -v is the heaviest path.
_gx_tree
ucheck "git exec-path: the exec-path is resolved at most ONCE per report (cached, not per row)" \
  "source '$UI'; source '$FN'; core-doctor -v >/dev/null; (( \$(grep -c -- '--exec-path' '$GXROOT/calls') == 1 ))" \
  PATH="$GXBIN" CORE_NO_PAGER=1
