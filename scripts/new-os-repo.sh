...
# beyond what CI already runs, so `make lint` == the reusable lint gate in dotfiles-core.
#
...
lint: shellcheck syntax zsh-syntax capabilities ## The gate: shellcheck + bash -n + zsh -n + the capability schema (what CI runs)
...
# in a --no-vendor scaffold), so the test is honest in both states.
for pair in "core/zsh/loader.zsh:$CFG/zsh/loader.zsh" "core/starship/starship.toml:$CFG/starship.toml" \
  "core/nvim:$CFG/nvim" "core/git/gitconfig:$tmp/home/.gitconfig"; do
  src="$REPO/${pair%%:*}"
  dest="${pair#*:}"
  [[ -e "$src" ]] || continue
  check_link "$dest" "$src"
