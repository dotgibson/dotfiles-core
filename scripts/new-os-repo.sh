...
# vendor set, through lib/core-vendor.sh's producer, not `git subtree add` (#676) — then writes the entry ~/.zshenv + .zprofile pair, an os/<os>.zsh stub,
# a starter bootstrap on Core's bootstrap driver (blib_main, #976), a .gitignore, a Makefile carrying the fleet's canonical `make`
...
add), then write the ZDOTDIR entry pair (the managed ~/.zshrc loader is the driver's),
os/<os>.zsh, a starter bootstrap on Core's bootstrap driver, .gitignore, a Makefile with the fleet's make vocabulary, a test/ suite and
...
# ── entry layer (ZDOTDIR model): ~/.zshenv → ZDOTDIR; .zprofile in $ZDOTDIR ─────
# The interactive entry, $ZDOTDIR/.zshrc, is NOT written here: Core's bootstrap driver
# writes the managed ~/.zshrc loader (blib_write_zshrc_loader — the v4 numbered-fragment
# glob, one definition for the whole fleet) and seeds $ZDOTDIR/.zshrc as a symlink to it,
# exactly as the eight OS/Role repos on the driver get it (#976, #986). This scaffold used
# to carry its own zshrc.zsh copy of that loader — a second definition to drift.
#
...
# and the closing report all run from core/lib/bootstrap-lib.sh — the previous starter
# carried a hand copy of the link step (and of blib_link's backup shape, "keep the two
# in step") that the fleet had just finished retiring.
w "$TARGET/bootstrap.sh" <<EOF
#!/usr/bin/env bash
# bootstrap.sh — wire the vendored Core + the $OS os/ layer into place. Idempotent.
# Generated by dotfiles-core/scripts/new-os-repo.sh — extend with $OS provisioning.
#
# SHAPE: this file DECLARES what it is and DEFINES the hooks that are $OS's, then hands
...
# that ONE definition. Add $OS provisioning as a bootstrap_provision() hook (the driver runs
...
Wire the vendored Core + the $OS os/ layer into place. Idempotent; safe to re-run
...
# A scaffolded repo used to ship with NO auto-tag.yml at all — so it never cut a single
# release of its own, and the two version lines an OS repo is supposed to carry (the Core
# it vendors, and itself) collapsed to one. Stamp the caller, in the shape Core documents
# on auto-tag-call.yml (#696): a DENYLIST over the installable surface, so the repo
# releases its OWN work rather than only what a Core fan-out sweeps in, plus a dispatch
# that makes the other two SemVer components reachable. Same v-major discipline as the
# lint caller above — read, never typed.
w "$TARGET/.github/workflows/auto-tag.yml" <<EOF
name: auto-tag

# Cut THIS repo's own next vX.Y.Z tag (+ a GitHub Release) when its INSTALLABLE STATE
# changes on main. Delegates to dotfiles-core's reusable auto-tag-call.yml — one
# definition of the version math (scripts/auto-tag.sh), idempotent (a no-op when HEAD is
# already tagged vX.Y.Z). Generated by dotfiles-core/scripts/new-os-repo.sh at Core major
# v$_core_major.
#
...
# allowlist fails the way \`paths: ['core/**']\` failed across the fleet: the tag tracked
# the vendored dependency and native work was released only by coincidence. The denylist's
# failure mode is a spurious patch tag — noisy, not wrong. \`!\` patterns are evaluated in
# order, so the exclusions override the \`**\` above.
on:
  push:
    branches: [main]
    paths:
      - '**'
      # Docs and licence — never installed. BOTH markdown forms: GitHub's filter-pattern
      # semantics for \`**\` spanning directory separators are not pinned down citably, and
      # this repo will grow nested markdown (.github/ISSUE_TEMPLATE/*.md).
      - '!**.md'
      - '!**/*.md'
      - '!LICENSE'
      ...
      - '!.github/**'
      - '!test/**'
      # Author-time tooling config.
      - '!.editorconfig'
      - '!.markdownlint.jsonc'
      - '!.pre-commit-config.yaml'
      - '!renovate.json'
  # The third SemVer component, reachable without editing this file. Safe to press at any
  # time: the reusable no-ops when HEAD already carries a vX.Y.Z tag.
  workflow_dispatch:
    inputs:
      bump:
