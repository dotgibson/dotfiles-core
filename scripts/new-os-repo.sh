...
# could force-remove whatever an inherited `_wt` happened to name. The sync runs as an
...
# gate (shellcheck, bash -n, zsh -n) plus the capability schema. The gate itself is
# called by .github/workflows/lint.yml (scaffolded beside this file); its other legs —
# RETURN-trap discipline, actionlint, markdownlint, gitleaks and the Makefile-gate check
# — run there only, so a green `make lint` is necessary, not sufficient. Add local
# mirrors as the repo grows (dotfiles-Fedora's Makefile shows the shape for each).
#
...
which \`.github/workflows/test.yml\` also runs on every push; \`make lint\` is the shell
half of Core's reusable lint gate, which \`.github/workflows/lint.yml\` calls in full.
