...
# failed 0` for the one target; the capture is `|| true` so errexit cannot skip it.
#
...
# beyond what CI already runs. `make lint` reproduces Core's reusable lint gate locally,
# leg for leg — shellcheck, bash -n, zsh -n, RETURN-trap discipline, the capability
...
# gitleaks.toml, this repo's .markdownlint.jsonc, the pinned versions in
# core/scripts/tool-versions.env. The gate itself is called by .github/workflows/lint.yml
# (scaffolded beside this file). A leg whose tool is not installed SAYS SO and skips —
# never silently, and CI still runs it — so a green `make lint` on a box with every tool
# is a green gate, and on a bare box the skips name what was not checked.
#
...
lint: shellcheck syntax zsh-syntax trap-guard capabilities markdownlint actionlint secrets make-gate ## Reproduce CI's lint gate locally, leg for leg (a leg whose tool is absent says so and skips; CI still runs it)
...
which \`.github/workflows/test.yml\` also runs on every push; \`make lint\` reproduces
Core's reusable lint gate leg for leg — the same gate \`.github/workflows/lint.yml\` calls
— and a leg whose tool is not installed says so and skips (CI still runs it).
