...
# fix_cmd <repo> — the make target that actually regenerates THAT row.
#
# `make gen-hero-tape` rewrites the `.` row and nothing else, so naming it while reporting a
# SIBLING sends the reader to a command that leaves their file untouched — advice that
...
# shellcheck disable=SC2016  # the backticked make target below is TEXT for the rendered
# tape's reader, not a command substitution to expand here.
banner() {
  local fix
  fix="$(fix_cmd "$1")"
...
while IFS="$TAB" read -r repo out checkout sigcmd proof signature; do
  ...
  if [[ "$MODE" == check ]]; then
    if [[ ! -f "$file" ]]; then
      fail "$label/$out is missing — run: $(fix_cmd "$repo")"; rm -f "$tmp"; _bump 1; continue
    fi
    ...
    if core_files_identical "$file" "$tmp"; then
      ...
      printf '    fix: edit assets/hero.tape.in or assets/hero-repos.txt, then run: %s\n' "$(fix_cmd "$repo")" >&2
      _bump 1
    fi
    rm -f "$tmp"
  else
