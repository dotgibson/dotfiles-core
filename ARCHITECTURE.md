...
git fetch origin refs/tags/v6 && wt="$(mktemp -d)/core" &&   # one chain: a failed fetch stops here, never reusing a stale FETCH_HEAD
  ...
  if grep -Eq 'repos: +updated 1 +skipped 0 +failed 0 ' <<<"$out"; then rc=0; else rc=1; fi   # the summary is the verdict (see below); `|| true` and the `if` keep `set -e` from skipping the cleanup
