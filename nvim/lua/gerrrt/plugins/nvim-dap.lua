...
--
-- Mason's debugpy is preferred when present because it is already installed and offline. `uv` is
-- the fallback rather than bare python3 because uv is what Core already assumes for the Astral
-- stack (ty/ruff are `uv tool install`ed), and because Mason's debugpy needs a working
-- `python3 -m venv` — which on Debian/Kali means python3-venv and on Alpine py3-pip, i.e. a
-- per-distro packaging line we would rather not add to PORTING-MATRIX.md.
